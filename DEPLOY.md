...
Файлы, принадлежащие root (включая `/etc/judgehelper/judgehelper.env`),
меняет администратор сервера; порядок доступа описан вне репозитория.
...
* адрес сервера лежит в secret `DEPLOY_HOST`, а его хост-ключ — в secret
  `DEPLOY_KNOWN_HOSTS` (строка формата `known_hosts`, см. `ssh-keyscan`);
  неизвестный ключ не принимается.
...
Сайт обслуживается на `judgehelper.ru`. DNS-записи указывают на сервер:

| Тип | Имя                  | Значение       |
|-----|----------------------|----------------|
| A   | `judgehelper.ru`     | `<IP сервера>` |
| A   | `www.judgehelper.ru` | `<IP сервера>` |
